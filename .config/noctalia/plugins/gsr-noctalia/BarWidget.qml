...
NIconButton {
    ...
    NPopupContextMenu {
        ...
        model: {
            ...
            // 24/7 toggle
            items.push({
                "label": mainInstance?.replayAlwaysOn
                    ? qsTr("Disable 24/7 replay")
                    : qsTr("Enable 24/7 replay"),
                "action": "toggle-always-on",
                "icon": mainInstance?.replayAlwaysOn ? "toggle-right" : "toggle-left"
            });
        ...
        onTriggered: action => {
            ...
            } else if (action === "toggle-always-on" && mainInstance) {
                mainInstance.setAlwaysOnReplay(!mainInstance.replayAlwaysOn);
            }
        }
    }
}
